...
def compose_matrix(scale=None, shear=None, quaternion=None, angles=None, translate=None, perspective=None):
    ...
    Sequence of transformations:
        scale : vector of 3 scaling factors
        shear : list of shear factors for x-y, x-z, y-z axes
        quaternion : quaternion [x, y, z, w] for rotation
        translate : translation vector along x, y, z axes
        perspective : perspective partition of matrix

    >>> scale = numpy.random.random(3) - 0.5
    >>> shear = numpy.random.random(3) - 0.5
    >>> quaternion = numpy.random.random(4) - 0.5
    >>> trans = numpy.random.random(3) - 0.5
    >>> persp = numpy.random.random(4) - 0.5
    >>> M0 = compose_matrix(scale, shear, quaternion, trans, persp)
    ...
    if translate is not None:
        T = translation_matrix(translate[:3])
        M = numpy.dot(M, T)
    if quaternion is not None:
        R = quaternion_matrix(quaternion)
        M = numpy.dot(M, R)
    if shear is not None:
        S = shear_matrix(shear)
        M = numpy.dot(M, S)
    if angles is not None:
        R = euler_matrix(angles[0], angles[1], angles[2], 'sxyz')
        M = numpy.dot(M, R)
    if scale is not None:
        Sc = scale_matrix(scale)
        M = numpy.dot(M, Sc)
    M /= M[3, 3]
    return M
