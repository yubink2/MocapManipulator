...
# load human
humanoid = bc.loadURDF("urdf/humanoid_for_rwe.urdf", (0, 0.03, 0), globalScaling=0.25, useFixedBase=True)
left_shoulder = 6
left_elbow = 7
human_rest_poses = [0.0, 0.0]
baseOrn = bc.getQuaternionFromEuler((1.57, 0, 1.57))
bc.resetBasePositionAndOrientation(humanoid, (0, 0.3, 0), baseOrn)

for j in range(bc.getNumJoints(humanoid)):  # change colors of the human model limbs
    ji = bc.getJointInfo(humanoid, j)
    bc.changeDynamics(humanoid, j, linearDamping=0, angularDamping=0)
    bc.changeVisualShape(humanoid, j, rgbaColor=[1, 1, 1, 1])

# load robot
robot = UR5Robotiq85(bc, (-0.8, 0, 0.5), (-1.57, 0, 0), globalScaling=1.2)
robot.load()
robot.reset()
...
# np.save("pc_human_scene_rwe.npy", dict)
np.save("pc_human_scene_rwe.npy", pc_normals)
